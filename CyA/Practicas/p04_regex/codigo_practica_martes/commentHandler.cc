...
void commentHandler::StoreMultiLineComments(const int first_comment_line, const int last_comment_line, const std::string& multiline_comment) {
  if (first_comment_line == 1) {
   ...
   comments_.push_back("[Lines " + std::to_string(first_comment_line) + " - " + std::to_string(last_comment_line) + " ] MULTILINE COMMENT:" + multiline_comment + "\n     */");
  }
}
