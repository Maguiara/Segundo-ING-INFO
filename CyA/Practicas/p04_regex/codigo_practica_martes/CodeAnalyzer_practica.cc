...
void CodeAnalyzer::Analyze(const std::string& inputfile, const std::string& outputfile) {
  ...
  while (std::getline(input, line)) {
    ...
    if (std::regex_search(line, loopRegex)) {
      ...
      AnalyzeStructs(line, match, lineNumber, structhandler);
    }
  }

  WriteOutput(inputfile, outputfile, variables, loops, comments, structhandler);
  input.close();
}
 ...
 */
void CodeAnalyzer::WriteOutput(const std::string& inputFile, const std::string& outputFile, variableHandler& variables, loopsHandler& loops, commentHandler& comments, structHandler& structhandler) {
  ...
  output << "\n STRUCTS \n ";
  structhandler.PrintStructs(output);
